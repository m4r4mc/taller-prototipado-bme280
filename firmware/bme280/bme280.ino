...
void loop() {
   ...
   if (tiempoActual - tiempoThingSpeak >= intervaloThingSpeak) {
      ...
      int respuesta = ThingSpeak.writeFields(myChannelNumber, WriteAPIKey); 
      
      if(respuesta == 200){
        Serial.println("Actualizado en ThingSpeak");
      } else {
        Serial.println("Error al actualizar ThingSpeak. HTTP error: " + String(respuesta));
      }
   }
}
